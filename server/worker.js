 ...
 * - SITE_URL (e.g. https://USERNAME.github.io/REPO)
 */
export default {
  async fetch(request, env) {
    ...
    if (!env.STRIPE_SECRET_KEY) return new Response("Missing STRIPE_SECRET_KEY", { status: 400, headers: cors });
    if (!env.SITE_URL) return new Response("Missing SITE_URL", { status: 400, headers: cors });

    let body;
    try { body = await request.json(); } catch { return new Response("Invalid JSON body", { status: 400, headers: cors }); }
    ...
    const allowed = ["SI"];
    for (const c of allowed) params.append("shipping_address_collection[allowed_countries][]", c);

    params.set("line_items[0][price_data][currency]", "eur");
    params.set("line_items[0][price_data][product_data][name]", "Floating bed turnkey (promo)");
    ...
    for (const a of addons) {
      ...
      params.set(`line_items[${idx}][price_data][product_data][name]`, `Add-on: ${a}`);
    ...
    if (!stripeRes.ok) return new Response(text, { status: 400, headers: cors });

    let session;
    try { session = JSON.parse(text); } catch { return new Response("Stripe response parse error", { status: 400, headers: cors }); }
